...
The engines that recognise and speak **on this machine** are a second, optional
step — about 30 MB, exactly as on Windows: **Settings ▸ Voice**, choose local, press
**Install**. `run.sh` says so on its first run, so it is not something to discover
